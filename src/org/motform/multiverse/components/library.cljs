...
(defn new-story-item []
  [:section.library-item-new-story-card.v-stack.centered
   [:a.library-item-new-story.shadow-large.tooltip-container
    {:href (routes/url-for :new-story)
     :on-pointer-down #(do (rf/dispatch [:active-page :new-story])
                           (. (.-history js/window) pushState #js {} "" (routes/url-for :new-story)))} ; routing schmouting
    "+"
